#!/usr/bin/env node
// Syna v0.7 acceptance orchestrator (the v0.6 gate with the 0.7 evidence: no @deprecated item in the API inventory,
// the inventory diff against the 0.6.0 record, the `any` budget against the 0.6.0 baseline, the same-session benchmark
// comparison against the 0.6.0 source).
//
//   node scripts/verify-v07.mjs --dev       G0: build + type tests + core/regression + real PostgreSQL/FS + app matrix + tooling
//                                            + API inventory (0 deprecated items; diff against the 0.6.0 record) + same-machine benchmark comparison
//                                            + `any` budget + benchmarks
//   node scripts/verify-v07.mjs --release   G0 + G1: source archive, rebuild from the archive in an empty dir, pack + consumer smoke,
...
const validationName = release ? 'v0.7-release' : 'v0.7-dev'
...
// The 0.6.0 side of the final 0.6 release run's same-session comparison, measured on the baseline machine
// (validation/v0.6-release/benchmark-compare/current-same-session.json): the recorded 0.6.0 baseline.
const BENCHMARK_BASELINE = 'benchmarks/results-v0.6.0-baseline-same-machine.json'
// The 0.6.0 source: the evidence commit of the final 0.6 release run (identical core to the released f019402).
...
const ANY_BASELINE = 'scripts/any-baseline-v0.6.0.json'
const INVENTORY_BEFORE = 'work/v07/API_INVENTORY_BEFORE.json'
...
async function developmentGate() {
  ...
  // The gate's own tooling plus the v0.7 assertions: empty deprecation register, no-old-names scan, README example, API inventory, `any` budget.
  await run('gate-self-tests', 'node', ['--test', '--test-reporter=tap', ...glob('scripts/tests', '.test.mjs')], { noSkip: true })
  // v0.7 evidence: the public API inventory of this source, the gate's own assertion that no item of it is
  // deprecated (A11), and its diff against the 0.6.0 record when the record is present.
  ...
  if (existsSync(path.join(root, INVENTORY_BEFORE))) {
    ...
    steps.push({ name: 'api-inventory-diff', ok: true, exitCode: 0, mustRun: false, command: 'internal', log: path.relative(root, path.join(validationDir, 'api-inventory.json')), note: `${INVENTORY_BEFORE} is not part of this tree (the 0.6.0 record lives in the source repository); the inventory of this source was recorded` })
    log(`skip api-inventory-diff (${INVENTORY_BEFORE} absent; not a test)`)
  }
  // v0.7 evidence: `any` per file at or under the 0.6.0 baseline (A09 measured from 0.6.0, the last release).
  ...
  // v0.7 evidence: same-machine comparison with 0.6.0: every p50/p95 within ±10 %, every plan-cache counter equal.
  // Same session when the 0.6.0 commit can be exported (both sides measured under the same machine state, interleaved
  // rounds); else the recorded baseline file, and only where the host matches the machine it was recorded on.
  ...
  if (baselineExportable) {
    await run('benchmark-compare', 'node', ['scripts/benchmark-same-session.mjs', '--commit', BASELINE_COMMIT, '--baseline-label', '0.6.0', '--record', BENCHMARK_BASELINE, '--runs', String(BENCHMARK_RUNS), '--out-dir', path.join(validationDir, 'benchmark-compare')], { expectStdout: output => /^SAME-SESSION BENCHMARK COMPARISON OK$/m.test(output) })
  }
...
async function releaseGate(sourceFingerprint) {
  ...
  const archiveBase = `syna-v0.7.0-source`
  ...
  const rebuildDir = await mkdtemp(path.join(tmpdir(), 'syna-v07-rebuild-'))
  ...
  // The consumer uses the 0.7 surface only (`limits`, `anchor`, `reuse`, `isSynaError` narrowing, `env.inspect().abandonedAttempts`)
  // against the packed declarations; an expired 0.6 alias would not compile.
...
console.log(JSON.stringify({ result, revision: Doubler.version, explainOk: explanation.ok, missing, abandoned }))
...
log(`Syna v0.7 verify (${release ? 'release' : 'dev'}) — ${sourceFingerprint.files} source files, fingerprint ${sourceFingerprint.digest}`)
...
const manifest = {
  name: 'Syna v0.7.0 + Hyla-mini',
  version: packageJson.version,
  gate: 'scripts/verify-v07.mjs',
  status,
