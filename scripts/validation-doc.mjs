...
//
//   node scripts/validation-doc.mjs [validation/v0.8-release] [docs/VALIDATION.md]
//
...
const runDir = process.argv[2] ?? 'validation/v0.8-release'
...
const sessionBaselineFile = ['baseline-v0.7.0-same-session.json', 'baseline-v0.6.0-same-session.json', 'baseline-v0.5.0-same-session.json'].map(name => path.join(runDir, 'benchmark-compare', name)).find(file => existsSync(path.resolve(root, file)))
...
const diffText = diffSummary ? `${diffSummary[1]} items in the 0.7.0 record, ${diffSummary[2]} here: ${diffSummary[3]} added, ${diffSummary[4]} removed, ${diffSummary[5]} changed in signature, ${diffSummary[6]} changed in JSDoc only, ${diffSummary[7]} newly deprecated` : 'no diff recorded'
