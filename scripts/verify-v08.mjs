#!/usr/bin/env node
// Syna v0.8 acceptance orchestrator (the v0.7 gate with the 0.8 evidence: no @deprecated item in the API inventory,
// the inventory diff against the 0.7.0 record, the rename codemod idempotent over the migrated tree, no pre-0.8
// reference token in the core, the `any` budget against the 0.7.0 baseline, the same-session benchmark comparison
// against the 0.7.0 source).
//
//   node scripts/verify-v08.mjs --dev       G0: build + type tests + core/regression + real PostgreSQL/FS + app matrix + tooling
//                                            + API inventory (0 deprecated items; diff against the 0.7.0 record) + codemod idempotency
//                                            + old-token scan + same-machine benchmark comparison + `any` budget + benchmarks
//   node scripts/verify-v08.mjs --release   G0 + G1: source archive, rebuild from the archive in an empty dir, pack + consumer smoke,
...
const validationName = release ? 'v0.8-release' : 'v0.8-dev'
...
// The 0.7.0 side of the final 0.7 release run's same-session comparison, measured on the baseline machine
// (validation/v0.7-release/benchmark-compare/current-same-session.json): the recorded 0.7.0 baseline. It spells the
// pre-0.8 record keys; scripts/benchmark-compare.mjs reads both sides in the 0.8 spelling.
const BENCHMARK_BASELINE = 'benchmarks/results-v0.7.0-baseline-same-machine.json'
// The 0.7.0 source: the evidence commit of the final 0.7 release run (identical core to the released 1abe595).
...
const INVENTORY_BEFORE = 'work/v08/API_INVENTORY_BEFORE.json'
...
async function developmentGate() {
  ...
  // v0.8 evidence: the public API inventory of this source, the gate's own assertion that no item of it is
  // deprecated (A11 in 0.7, a success criterion of 0.8), and its diff against the 0.7.0 record when the record is present.
  ...
  if (existsSync(path.join(root, INVENTORY_BEFORE))) {
    ...
    steps.push({ name: 'api-inventory-diff', ok: true, exitCode: 0, mustRun: false, command: 'internal', log: path.relative(root, path.join(validationDir, 'api-inventory.json')), note: `${INVENTORY_BEFORE} is not part of this tree (the 0.7.0 record lives in the source repository); the inventory of this source was recorded` })
  ...
  // v0.8 evidence: same-machine comparison with 0.7.0: every p50/p95 within ±10 %, every plan-cache counter equal.
  // Same session when the 0.7.0 commit can be exported (both sides measured under the same machine state, interleaved
  ...
  if (baselineExportable) {
    await run('benchmark-compare', 'node', ['scripts/benchmark-same-session.mjs', '--commit', BASELINE_COMMIT, '--baseline-label', '0.7.0', '--record', BENCHMARK_BASELINE, '--runs', String(BENCHMARK_RUNS), '--out-dir', path.join(validationDir, 'benchmark-compare')], { expectStdout: output => /^SAME-SESSION BENCHMARK COMPARISON OK$/m.test(output) })
  }
...
async function releaseGate(sourceFingerprint) {
  ...
  const archiveBase = `syna-v0.8.0-source`
  ...
  const rebuildDir = await mkdtemp(path.join(tmpdir(), 'syna-v08-rebuild-'))
...
log(`Syna v0.8 verify (${release ? 'release' : 'dev'}) — ${sourceFingerprint.files} source files, fingerprint ${sourceFingerprint.digest}`)
...
const manifest = {
  name: 'Syna v0.8.0 + Hyla-mini',
  version: packageJson.version,
  gate: 'scripts/verify-v08.mjs',
  status,
