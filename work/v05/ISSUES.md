...
- Fix: K05/D07 — sync `read()`, deprecated `load()` returns `Awaited<T>`. Status: OPEN.
...
- Fix: D04/D05 remove barrier. Status: OPEN.
...
- Probe: "R04 Promise.race fallback" → FAIL (timeout 300ms). Fix: D04. Status: OPEN.
...
- Fix: D08 — reject with OWNER_NOT_READY. Status: OPEN.
...
- Fix: D09 realm closure. Status: OPEN.
...
- Fix: D03 — npm `semver`. Status: OPEN.
...
- Fix: in Attempt sequencing. Status: OPEN.
...
- Fix: D06 deadline diagnostics. Status: OPEN.
