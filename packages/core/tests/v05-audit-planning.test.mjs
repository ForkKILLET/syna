...
test('F-CD-02 a failure every candidate shares keeps its own code instead of UNSATISFIABLE_TOPOLOGY, independent of requires order', async () => {
  ...
  const ShareOnly = define.entry('share-only', { requires: { cache: Cache }, parameters: { tenant: Tenant }, scope: { share: [Cache] } })
  const ShareWithAuto = define.entry('share-with-auto', { requires: { automatic: AutoUser, cache: Cache }, parameters: { tenant: Tenant }, scope: { share: [Cache] } })
