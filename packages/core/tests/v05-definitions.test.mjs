...
test('K01 Runtime construction is closed and inert: no Env, slot or instance; unknown definitions are refused; invalid options are TypeErrors', async () => {
  ...
  assert.throws(() => createRuntime({ services: [Eager], initialization: { deadlineMs: -1 } }), TypeError)
  assert.throws(() => createRuntime({ services: [Eager], planning: { searchBudget: 0 } }), TypeError)
  assert.throws(() => createRuntime({ services: [Eager], planCache: { maxEntries: 0 } }), TypeError)
