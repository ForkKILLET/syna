...
export function parseVersion(input) {
    ...
    if (!match)
        throw new TypeError(`Invalid semantic version: ${input}`);
    return {
...
function compareTuple(version, target) {
...
export function satisfiesVersion(versionText, rangeText) {
    ...
    if (range.startsWith('^')) {
        ...
        if (base.major > 0)
            return lower && version.major === base.major;
        if (base.minor > 0) {
    ...
    if (wildcard) {
        ...
        if (major !== 'x' && major !== '*' && version.major !== Number(major))
            return false;
        if (minor !== undefined && minor !== 'x' && minor !== '*' && version.minor !== Number(minor))
            return false;
        if (patch !== undefined && patch !== 'x' && patch !== '*' && version.patch !== Number(patch))
            return false;
        return true;
    }
    if (/[<>]=?/.test(range)) {
