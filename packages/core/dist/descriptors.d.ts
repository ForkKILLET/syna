...
export interface ImplementationSelectorDependency<C extends Contract<any> = Contract<any>> {
...
export interface PersistentImplementationRef<C extends Contract<any> = Contract<any>> {
...
export type ServiceFamilyApi<F> = F extends ServiceFamily<infer Api> ? Api : never;
...
export interface Binding<C extends Contract<any> = Contract<any>> {
    ...
    readonly contract: C;
    readonly metadata: Readonly<DescriptorMetadata>;
    to<S extends ServiceRevision<any>>(service: ServiceInstance<S> extends ContractApi<C> ? S : never, version?: string): PersistentImplementationRef<C>;
 ...
 */
export type ServiceFailurePolicy = 'sticky' | {
...
export type Dependency = ServiceRevision<any> | ServiceRange<any> | Contract<any> | Input<any> | Binding<any> | AutoImplementation<any> | ImplementationSelectorDependency<any> | AllImplementations<any> | EntryDescriptor<any, any> | ForwardDependency<any>;
export type DependencyMap = Readonly<Record<string, Dependency>>;
export type UnwrapForward<D> = D extends ForwardDependency<infer Inner> ? UnwrapForward<Inner> : D;
export type DependencyOutput<D> = UnwrapForward<D> extends ServiceRevision<infer Instance> ? Instance : UnwrapForward<D> extends ServiceRange<infer Family> ? ServiceFamilyApi<Family> : UnwrapForward<D> extends Contract<infer Api> ? Api : UnwrapForward<D> extends Input<infer T> ? T : UnwrapForward<D> extends Binding<infer C> ? ContractApi<C> : UnwrapForward<D> extends AutoImplementation<infer C> ? ContractApi<C> : UnwrapForward<D> extends ImplementationSelectorDependency<infer C> ? ImplementationSelector<C> : UnwrapForward<D> extends AllImplementations<infer C> ? ImplementationSet<C> : UnwrapForward<D> extends EntryDescriptor<any, any> ? BoundEntry<UnwrapForward<D>> : never;
...
export interface ServiceDefinition<Requires extends DependencyMap, Provides extends readonly Contract[], Instance> {
    ...
    readonly setup: (dependencies: DependencyRefs<Requires>, lifecycle: ServiceLifecycle) => Awaitable<Instance>;
}
...
export interface ServiceRevision<Instance = unknown> {
    ...
    readonly metadata: Readonly<DescriptorMetadata>;
    setup(dependencies: DependencyRefs<DependencyMap>, lifecycle: ServiceLifecycle): Awaitable<Instance>;
    range(version?: string): ServiceRange<ServiceFamily<Instance>>;
}
export type ServiceInstance<S> = S extends ServiceRevision<infer Instance> ? Instance : never;
...
export interface ImplementationCandidate<C extends Contract<any> = Contract<any>> extends ImplementationDescriptor<C> {
    readonly ref: CandidateRef<C>;
    readonly availability: CandidateAvailability;
}
export type AvailableImplementationCandidate<C extends Contract<any> = Contract<any>> = ImplementationCandidate<C> & {
...
export interface ImplementationSelector<C extends Contract<any> = Contract<any>> extends Iterable<ImplementationCandidate<C>> {
    readonly contract: C;
...
export interface ImplementationSet<C extends Contract<any> = Contract<any>> extends Iterable<ImplementationCandidate<C>> {
    readonly contract: C;
    readonly candidates: readonly ImplementationCandidate<C>[];
    resolve(ref: PersistentImplementationRef<C>): ImplementationCandidate<C>;
    load(candidate: ImplementationCandidate<C> | CandidateRef<C> | PersistentImplementationRef<C>): Promise<ContractApi<C>>;
}
export type EntryParameter = Input<any> | Binding<any>;
export type EntryParameterMap = Readonly<Record<string, EntryParameter>>;
export type BindingAssignment<B extends Binding<any>> = PersistentImplementationRef<B['contract']> | ServiceRevision<ContractApi<B['contract']>>;
export type EntryParameterValue<P extends EntryParameter> = P extends Input<infer T> ? T : P extends Binding<any> ? BindingAssignment<P> : never;
...
export interface EntryDescriptor<Requires extends DependencyMap = DependencyMap, Parameters extends EntryParameterMap = EntryParameterMap> {
    readonly kind: 'entry';
    readonly package: PackageDescriptor;
    readonly id: string;
    readonly apiVersion: number;
...
export type EntryParameters<E extends EntryDescriptor<any, any>> = EntryParameterValues<E['parameters']> & {
    readonly scope?: DeriveOptions;
};
export type EntryArguments<E extends EntryDescriptor<any, any>> = keyof E['parameters'] extends never ? [parameters?: EntryParameters<E>] : [parameters: EntryParameters<E>];
export type EntryDependencies<E extends EntryDescriptor<any, any>> = DependencyRefs<E['requires']>;
export type EntryCallback<E extends EntryDescriptor<any, any>, Result> = (dependencies: EntryDependencies<E>, env: EnvHandle<E['requires']>) => Awaitable<Result>;
export type EntryRunArguments<E extends EntryDescriptor<any, any>, Result> = keyof E['parameters'] extends never ? [callback: EntryCallback<E, Result>] | [parameters: EntryParameters<E>, callback: EntryCallback<E, Result>] : [parameters: EntryParameters<E>, callback: EntryCallback<E, Result>];
...
export interface ServiceOverride<From extends ServiceRevision<any> = ServiceRevision<any>, To extends ServiceRevision<any> = ServiceRevision<any>> {
...
export type InspectionNodeKind = 'service' | 'input' | 'binding' | 'selector' | 'all' | 'entry';
...
export type EntryCheck = {
    readonly ok: true;
    readonly inspection: PlannedEnvInspection;
} | {
    readonly ok: false;
...
type ContractApiUnion<Provides extends readonly Contract[]> = Provides[number] extends infer C ? C extends Contract<infer Api> ? Api : never : never;
...
export type ProvidedShape<Provides extends readonly Contract[]> = [
    Provides[number]
] extends [never] ? unknown : UnionToIntersection<ContractApiUnion<Provides>>;
export interface EntryDefinition<Requires extends DependencyMap, Parameters extends EntryParameterMap> extends DefinitionOptions {
    readonly requires?: Requires;
    readonly parameters?: Parameters;
    readonly scope?: DeriveOptions;
}
export interface PackageDefinitions {
    ...
    service<const Requires extends DependencyMap = {}, const Provides extends readonly Contract[] = readonly [], Instance extends ProvidedShape<Provides> = ProvidedShape<Provides>>(definition: ServiceDefinition<Requires, Provides, Instance>): ServiceRevision<Instance>;
    service<const Requires extends DependencyMap = {}, const Provides extends readonly Contract[] = readonly [], Instance extends ProvidedShape<Provides> = ProvidedShape<Provides>>(name: string, definition: ServiceDefinition<Requires, Provides, Instance>): ServiceRevision<Instance>;
