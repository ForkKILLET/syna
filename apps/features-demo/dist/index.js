...
import { createRuntime, definePackage, forward, } from '@syna/core';
...
A = define.service('cycle-a', {
    ...
    setup({ b }) {
        return { name: 'a', callB: async () => (await b.load()).name };
    },
});
B = define.service('cycle-b', {
    requires: { a: forward(() => A) },
    setup({ a }) {
        return { name: 'b', callA: async () => (await a.load()).name };
    },
});
...
console.log('Structural cycle works after setup:', await (await root.deps.a.load()).callB(), await (await root.deps.b.load()).callA());
const child = await root.enter(Child, { epoch: Symbol('child') });
const childCounter = await (await child.deps.consumer.load()).counter();
console.log('Re-providing an Input forks its reverse closure:', childCounter.id !== rootCounter.id);
await child.dispose();
await root.dispose();
...
try {
    ...
    console.log('Setup wait cycle rejected:', error instanceof Error ? error.message : String(error));
}
...
try {
    ...
    console.log('Lineage-fixed conflict rejected:', error instanceof Error ? error.message : String(error));
}
