...
import { AppEntry, ArticleSummarizer, ArticleSummaryLlm, BlogEntry, BlogRuntime, HylaApplication, ProviderPanel, ProvidersEntry, RequestEntry, } from '@syna-demo/hyla';
...
await blogEnv.run(ProvidersEntry, { call: { requestId: 'provider-panel', blogId: 'blog-42' } }, async ({ panel }) => {
    ...
    const legacyOpenAi = candidates.find(candidate => candidate.familyId === OpenAIv1.family.id
        && candidate.version === OpenAIv1.version);
    if (!legacyOpenAi)
        throw new Error('Expected OpenAI v1 candidate.');
    console.log(await providerPanel.run(legacyOpenAi.persistentRef, 'Run a provider selected from the canonical selector slot.'));
});
console.log('Runtime catalog (no Env topology required):', runtime.catalog.implementations(LlmConnector).map(item => `${item.familyMetadata.displayName}@${item.version}`));
