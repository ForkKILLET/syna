...
| M-28 | Hyla-mini 站点配置与渲染 API：`saveSiteConfig`/`getSiteConfig`、`PipelineBuilder`、`MarkdownStageFactory`、`SiteContext.cacheStats` | 配置不校验；`build(document)`；`factorySetupCounts` 计数；`stats.configured`；页面缓存无界 | 两端校验（`SiteConfigError` `INVALID_SITE_CONFIG`：schema、`isSafeHref`、`isCssColor`、域名、租户 id）；`build(document, { trust })`（评论为 `untrusted`，见 PLUGIN_AUTHORING）；`factorySetupCounts` 冻结且弃用，改用 `factoryInstances()` / `stats.instance`；`sanitizer` 角色；`PIPELINE_CACHE_MAX_ENTRIES`；`siteManager.pageCacheMaxEntries`（默认 256）；`cacheStats` 新增 `coalesced/evictions/maxEntries` | 以前能保存的坏文档（`javascript:` href、非颜色 accent、少于三个阶段的配方、多余键）现在被拒绝：先修数据；第三方 sanitizer Factory 要声明 `sanitizer: { options }`（第三轮 I-72…I-75） |
| M-29 | Hyla-mini 静态构建：`StaticBuilder.build()`、`StaticBuildManifest`、`startStaticServer` | 先删后写、无锁；清单无内容版本；输出目录只做前缀检查，跟随符号链接 | 先在内存中从同一内容版本渲染，再逐文件原子替换、删除过期文件、最后写清单；`StaticBuildManifest` 新增 `contentVersion`、`attempts`，`outputDir` 为解析后的真实路径；`StaticBuildError`（`UNSAFE_OUTPUT_DIR`/`OUTPUT_DIR_NOT_EMPTY`/`BAD_MANIFEST`/`BUILD_LOCKED`/`BUILD_CONTENT_CHANGED`）；构建期间目录里有 `.hyla-build.lock`；静态服务器根目录必须存在（`realpath`） | 输出目录本身或其下任何路径是符号链接的部署现在被拒绝；不要把 `.hyla-build.lock` 当作构建产物（第三轮 I-76、I-77） |
| M-30 | Hyla-mini 后端：PostgreSQL schema、`repositoryOn()`、`DatabasePool.withClient()`、`PoolStats` | `posts` 主键 `id`；变更与版本推进是两条语句；域名冲突只查 `sites` 的 JSON；任何错误都销毁连接 | 主键 `(tenant_id, id)`（启动迁移幂等地改旧 schema）；新增 `domains` 表（迁移回填）；`repositoryOn(tenantId, sql, runMutation?)`，公共路径每个变更自成事务；文件系统后端有 `content.version.pending` 标记；只有连接级错误销毁连接，`stats().removed` | 依赖 `id` 全局唯一的外部程序要改按 `(tenant_id, id)`；直接写 `sites` 的程序要同步 `domains`（或让租户重新保存配置）；自己调用 `repositoryOn` 的程序要传 `runMutation`（第三轮 I-78…I-81） |
...
| M-31 | core 生命周期：`inspect().unsettledAttempts`、`attempt-abandoned` 事件、`UNSETTLED_ATTEMPT` 详情、`run()` 的关闭错误 | 账本状态只有 `timed-out`/`abandoned`，弱引用；关闭报告一律说 "still running"；`run()` 回调成功但关闭报错时结果丢失 | 账本状态增加 `rolling-back`（setup 已结束、回滚超出 grace）与 `settling`（迟到清理进行中），强引用 attempt 直到结算；`runtime.dispose()` 先给正在结算的清理最多 `disposal.graceMs` 再报告；报告写明阶段（`details.slots[].phase`、事件的 `phase`，文案 "still rolling back" / "still running or rolling back"）；关闭错误带不可枚举的 `result` | 匹配 "still running" 文案或只认两种账本状态的代码要改；需要结果的 `run()` 调用者读 `error.result`（复审 I-89、I-92） |
| M-32 | core 规划：`check()`/`explain()` 的 id、`requires` 解析顺序、模板命中时的漂移检查、selector 候选缓存 | 规划消耗 `slot-N`；按字面量插入顺序解析；模板命中不检查漂移（漂移副本静默执行规范副本的 setup）；候选 Entry 缓存按 Contract 全局共享 | 规划用 `check-slot-N`/`check-choice-N`；按 key 排序解析（只调换 `requires` 顺序的副本得到同一拓扑，slot 编号可能变化）；命中也诊断 `DUPLICATE_DEFINITION`，`check()`/`explain()` 与 `enter()` 一样抛出定义错误；候选缓存按物理描述符 | 依赖 slot 编号或 key 顺序的测试要改；曾靠缓存"绕过"漂移的定义要修正（I-85、I-86、I-88、I-93） |
| M-33 | Hyla-mini 站点管理器与渲染：`SiteManagerClosedError`、`SiteContext`、`StaticBuildError('BAD_MANIFEST')`、`isSafeHref`、不可信构建 | 被关闭截断的创建抛 `INVALID_ENV_STATE` 并计入失败；`renderSnapshot` 逐篇回读；任何带 `files` 的 JSON 都算上次构建；反斜杠 href 通过；追加的 sanitizer 与配方的 `finalPass` 合并 | `SiteManagerClosedError({ cause })`；新增 `SiteContext.renderPostPage(post, principal, version)`；清单必须是本构建器为本租户写的；`isSafeHref` 拒绝反斜杠；每个 sanitizer 配置一个插件身份，构建器核对追加成功否则 `RecipeError` | 实现 `SiteContext` 的替身要加 `renderPostPage`；导航 href 含反斜杠的配置要改；自定义 sanitizer Factory 必须为每个配置产生独立的一遍（I-94、I-95、I-97、I-99、I-101） |
| M-34 | Hyla-mini 后端：`DatabaseConfig`、`DatabasePool`、`PoolStats`、事务错误、`listTenants()`、NUL | COMMIT 结果不检查；无每租户锁；工作单元内重入永远等待；`pool.end()` 无界；`stats()` 只有 `total/idle/waiting/removed`；文件系统接受 NUL；`listTenants` 只看 `sites ∪ posts` | `TransactionAbortedError`（`TRANSACTION_ABORTED`）、`TransactionReentrancyError`（`TRANSACTION_REENTRANCY`）、`PoolClosedError`、`PoolCloseTimeoutError`；`DatabaseConfig.lockTimeoutMs`（默认 30 s，55P03）/`closeTimeoutMs`（默认 5 s）；每个工作单元先取 `hyla-mini:tenant:<id>` advisory 锁；`PoolStats.leased/waiting`；两个后端拒绝 NUL（`TypeError`）；`listTenants()` 为六张表的并集 | 在工作单元里捕获 SQL 错误后继续的代码现在得到 `TransactionAbortedError`；在 `transaction()` 内再调公共仓库的代码要改用传入的仓库；长事务要考虑 `lockTimeoutMs`；含 NUL 的内容要先清洗（I-102…I-109、I-113） |
| M-35 | 交付：`scripts/verify-v05.mjs` 清单与步骤、`scripts/pg-test-cluster.mjs`、demo 输出、`npm run test:scripts` | 步骤只杀直接子进程；清单无 PostgreSQL 版本、无运行对比；demo 只靠退出码；集群脚本收到信号不停集群 | 步骤在独立进程组中运行（`scripts/lib/step-runner.mjs`：SIGTERM→SIGKILL→有界的 close 等待，步骤字段 `closeTimedOut`/`spawnError`/`note`）；清单 `environment.postgres`、`previousRun`；新增步骤 `gate-self-tests` 与 `rebuild-gate-self-tests`；四个 demo 打印 `demo: OK`（`demos` 步骤要求四行）；集群脚本打印 `pg-test-cluster: server … at …`，转发 SIGTERM/SIGINT 并以 128+信号退出 | 解析清单的工具要认识新字段；自己包装 `with` 的脚本注意退出码变化（I-111、I-112、I-115、I-116） |
