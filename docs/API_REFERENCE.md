...
`to()`/`parse()` produce an `ImplementationRef`, a JSON-safe preference: `{ kind: 'persistent-implementation-ref', contractId, familyId, version }`. `familyId` is the implementation family (`ServiceFamily.id`). Persisted-data compatibility: the 0.5 line serialized the same value under the key `implementationId`; `parse()` and every Runtime read path accept that key permanently (both keys, when equal), and each Runtime read of such a reference is reported once as the diagnostics event `legacy-implementation-ref` (`docs/MIGRATION_V06_TO_V07.md` §2). `kind` is the stable on-disk discriminator. A ref never points at an Env-local slot.
...
Options: `requires`, `provides`, `eager`, `uniqueWithin: 'lineage'`, `failure`, `setupDeadlineMs` (how long one `load()` waits on the current setup attempt before `INITIALIZATION_TIMEOUT`, overriding the Runtime default; the attempt keeps running; `Infinity` disables), `metadata`, `revisionMetadata`, `setup`.
...
- `policy`: `orderAutoCandidates(contract, candidates, context)` and `orderVersionCandidates(family, candidates, context)` return the candidates in preference order (a total order of the given list). `context.dependencySite` is the dependency site being resolved (`…/dependency:<name>` for a declared requirement, `…/persistent:<familyId>` for a resolved reference); `context.parentActiveRevisionKeys` are the revision keys active in the parent lineage. (The 0.5 name `context.site` was removed in 0.7.0.)
- `limits.setupDeadlineMs`: the default wait of one `load()` on the current setup attempt (per waiter; re-armed when a retry starts a new attempt, not counted during the `delayMs` backoff) → `INITIALIZATION_TIMEOUT` with `details.attemptStillRunning: true`, `details.pendingLoads` and an optional `details.suspectedWaitCycle` (an observation, not a proof). The deadline ends the wait, not the attempt: the slot stays `starting` (`env.inspect()` reports `overdueMs` for it), `attempt-overdue` is reported once per attempt, later `load()`s join the attempt with their own windows, a late success is adopted while the owner Env is `ready` (`late-setup-result` with `adopted: true`; nothing is cleaned up) and discarded only by a close (`adopted: false`, cleanups run), a late failure follows the failure policy (`late-setup-failure`). A timeout consumes no `failure.attempts` and triggers no `delayMs`. `enter()` is the waiter of each eager attempt: an eager setup that outlasts the deadline fails the activation (`ENTRY_ACTIVATION_FAILED`, `causeCode: 'INITIALIZATION_TIMEOUT'`, `causeDetails.slot` names the overdue slot) and the rollback close discards its late result. To wait less than the deadline use `load({ signal: AbortSignal.timeout(ms) })` (`LOAD_CANCELLED`); there is no other option.
- `limits.disposalGraceMs`: how long disposal waits, after broadcasting the stop signal, for each in-flight setup attempt of the closing Env (running or already timed out) to settle before abandoning it. Slots of one Env wait concurrently, so each Env's own close is bounded by one grace period regardless of `setupDeadlineMs` (even `Infinity`); descendants close first, so a tree in which every level owns a stuck attempt closes in at most one grace per level. At the end of the grace the close is complete whatever is still pending: the Env leaves the tree and the `inspect()` counts and its `state` is `'disposed'`. Each attempt that outlived the grace is *abandoned*, not an error of the close: `dispose()` fulfils, `attempt-abandoned` is reported once per attempt (`phase` is `'setup'` while the raw Promise is pending and `'rollback'` when the setup had settled and its cleanups outlived the grace; `dependencies` names the dependency slots the attempt may still use — they are closed in the normal order regardless), and the attempt is listed in `runtime.inspect().unsettledAttempts` and in `env.inspect().abandonedAttempts` until it settles late (discarded, cleaned up, `late-setup-*`) or its Promise is found unreachable (`attempt-unreachable`).
- `limits.planningBudget`: candidate expansions per plan before `PLANNING_BUDGET_EXCEEDED`.
- `limits.planCacheEntries`: plan template cache capacity (`inspect().planCache` reports `hits`, `misses`, `entries`, `evictions`, `maxEntries`). The values shown above are the defaults. The 0.5 nested records `planCache: { maxEntries }`, `initialization: { deadlineMs }`, `disposal: { graceMs }` and `planning: { searchBudget }` were removed in 0.7.0: giving one is a `TypeError` naming the limit to use, with or without `limits`.
- `diagnostics.onEvent`: `attempt-overdue` (`{ slot, revision, env, attempt, deadlineMs, elapsedMs }`: the first waiter on a setup attempt timed out; once per attempt), `late-setup-result` (`adopted: true` when a late success became the instance under a ready owner, `false` when a close discarded it and its cleanups ran), `late-setup-failure`, `attempt-abandoned` (`{ phase, slot, revision, env, elapsedMs, dependencies }`: the close of the owner Env ended while this attempt was still running or rolling back; `dependencies` lists the Service slots it may still use, with their state at that moment), `attempts-outstanding` (`{ attempts }`: reported once at the end of `runtime.dispose()` when the ledger is not empty; `attempts` is the `inspect().unsettledAttempts` view at that moment), `attempt-unreachable` (an overdue or abandoned attempt whose setup Promise was garbage-collected: nothing can settle it any more, so its cleanups ran and the attempt is closed; an overdue attempt's sequence then follows its failure policy), `foreign-thenable-setup`, `legacy-implementation-ref` (`{ contractId, familyId, version, site }`: the Runtime read an implementation reference in the 0.5 serialized form, `implementationId` instead of `familyId`; the reference is accepted, the event is reported once per read). Exceptions in the handler are ignored; diagnostics never change outcomes.
...
runtime.inspect()                     // admitted/internal/overridden services, root/live env counts, plan cache stats, warnings,
                                      // unsettledAttempts: attempts overdue (`timed-out`: still running under a live owner), abandoned, rolling back or settling late, held until they settle
...
env.inspect()                          // nodes with slot ids, owners and slot states; abandonedAttempts: the ledger entries this Env's close left behind
...
- `onDispose(cleanup)` is accepted for as long as the setup attempt is still executing, including after a waiter's deadline passed (the resource then belongs to the adopted instance and is released by disposal) or after its owner started closing (the late-settlement cleanup then runs it). A lifecycle whose setup Promise already settled is stale and refused (`LIFECYCLE_MISUSE`).
...
- `env.state` is advanced only by Runtime actions — `activating → ready → disposing → disposed` — and is `'disposed'` at the end of the bounded close whatever is still pending. An attempt that ignores the signal past the grace is abandoned and reported (`attempt-abandoned`); its dependencies are closed in the normal order anyway (the Runtime cannot revoke an instance it handed out) and the event names them. The Env leaves the tree and the Runtime's registries, so its parent no longer waits for it and `inspect()` no longer counts it. The attempt stays in `runtime.inspect().unsettledAttempts` and in `env.inspect().abandonedAttempts` until it settles late (discarded, cleaned up, `late-setup-*`) or becomes unreachable (`attempt-unreachable`); an attempt lives exactly as long as the caller's own setup Promise, never longer because of the Runtime. `dispose()` rejects only for errors of the close itself (a cleanup that threw); `runtime.dispose()` waits up to the grace for settling attempts and then reports the rest once as `attempts-outstanding`.
...
`C.all` yields an `ImplementationSet`: `candidates`, `resolve(ref)`, `load(candidate | candidateRef | ref, options?)` (`ref`: an `ImplementationRef`). Candidates are real nodes of the current Env; a `CandidateRef` belongs to one collection slot (`FOREIGN_CANDIDATE_REF` elsewhere). `ImplementationRef` (`{ kind, contractId, familyId, version }`) is JSON-safe; without the target family it fails with `MISSING_IMPLEMENTATION` — no supplier substitution.
...
| `INITIALIZATION_TIMEOUT` | one `load()` waited `setupDeadlineMs` on the current setup attempt; the attempt keeps running (adopted if the owner stays ready, discarded only by a close) | `{ slot, revision, env, attempt, deadlineMs, elapsedMs, pendingLoads: { revision, slot, state, waitingMs }[], suspectedWaitCycle?: string[], attemptStillRunning: true, note }` |
...
An attempt that outlives a close is not an error (0.7.0): the 0.6 code `UNSETTLED_ATTEMPT` was removed with S1/S2 (`docs/MIGRATION_V06_TO_V07.md` §3); the ledger (`inspect().unsettledAttempts`, `env.inspect().abandonedAttempts`) and the events `attempt-abandoned` / `attempts-outstanding` report it.
...
The 23 names the 0.6 line carried as deprecated aliases are gone, and the type declarations carry no `@deprecated` item: `EntryDefinition.scope` / `EntryDescriptor.scope` and `scope` inside the parameter record (use `reuse` and the options argument `{ reuse }`), `DeriveOptions` (`ReuseConstraints`), `ScopeTarget` (`ReuseTarget`), `env.bind()` (`env.anchor()`), `BoundEntry` (`AnchoredEntry`), `SynaRuntime` (`Runtime`), `DependencyRef` (`ServiceRef` / `InputRef`, `DependencyRefFor<D>` for a declared dependency), `PersistentImplementationRef` (`ImplementationRef`), `ref.implementationId` (`ref.familyId`), the nested option records `planCache` / `initialization` / `disposal` / `planning` with their four types (`limits`, `RuntimeLimits`) and `RuntimePolicyContext.site` (`dependencySite`). An expired form given at runtime — `scope` in a definition or in a parameter record, a nested option record — is a `TypeError` that names the current form. `docs/MIGRATION_V06_TO_V07.md` has the item-by-item table; the one permanent exception is the serialized key `implementationId`, which `parse()` and the Runtime read forever (see `ImplementationRef` above).
