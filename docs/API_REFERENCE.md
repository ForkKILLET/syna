...
- `diagnostics.onEvent`: `attempt-overdue` (`{ slot, revision, env, attempt, deadlineMs, elapsedMs }`: the first waiter on a setup attempt timed out; once per attempt), `late-setup-result` (`adopted: true` when a late success became the instance under a ready owner, `false` when a close discarded it and its cleanups ran), `late-setup-failure`, `attempt-abandoned`, `attempt-unreachable` (an overdue or abandoned attempt whose setup Promise was garbage-collected: nothing can settle it any more, so its cleanups ran and the attempt is closed; an overdue attempt's sequence then follows its failure policy), `foreign-thenable-setup`, `legacy-implementation-ref` (`{ contractId, familyId, version, site }`: the Runtime read an implementation reference in the 0.5 serialized form, `implementationId` instead of `familyId`; the reference is accepted, the event is reported once per read). Exceptions in the handler are ignored; diagnostics never change outcomes.
