...
- `diagnostics.onEvent`: `late-setup-result`, `late-setup-failure`, `attempt-abandoned`, `attempt-unreachable` (an abandoned or timed-out attempt whose setup Promise was garbage-collected: nothing can settle it any more, so its cleanups ran and the attempt is closed), `foreign-thenable-setup`, `legacy-implementation-ref` (`{ contractId, familyId, version, site }`: the Runtime read an implementation reference in the 0.5 serialized form, `implementationId` instead of `familyId`; the reference is accepted, the event is reported once per read). Exceptions in the handler are ignored; diagnostics never change outcomes.
