...
| D4 | `serviceRange(revision, range)` | `revision.range(range)`（同一对象、同一 origin）。**F2**：任务书提到的 `ServiceFamily.range()` 没有补上——`serviceRange` 的第一个参数是修订（range 的 origin），而 Family 上的 `range()` 会是一个没有 origin 的新依赖形式，改变私有域下的解析（0.5 第三轮 C1/C2），属于 §3.6 的"新 dependency 形式"，记入 `docs/DEFERRED.md` | type-tests | 无 |
